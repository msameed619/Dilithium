...
def load_expected_results():
    return load_json('VerifyT/expectedResults.json')

def main():
    data = load_json('VerifyT/prompt.json')
    expected_results = load_expected_results()

    # Filter the test group and expected results for tgId 5
    test_group = next((tg for tg in data['testGroups'] if tg['tgId'] == 5 and tg['testType'] == "AFT"), None)
    expected_group = next((tg for tg in expected_results['testGroups'] if tg['tgId'] == 5), None)

    if not test_group or not expected_group:
        print("Test group with tgId: 5 not found.")
        return
    ...
    for test, expected in zip(test_group['tests'], expected_group['tests']):
        if 61 <= test['tcId'] <= 75:
            ...
            expected_verification_result = expected['testPassed']
            ...
            print(f"Processing Test Case ID: {test['tcId']} with context: {context}")

            # Perform the signing operation
            verification = Verify(pk_bytes, message_bytes, signature_bytes, context_bytes)
            verification_result = verification

            # Compare the results
            if verification_result == expected_verification_result:
                result = "PASS"
            else:
                result = "FAIL"
