...
def load_expected_results():
    with open('KeyT/expectedResults.json', 'r') as file:
        return json.load(file)

def main():
    # Load the JSON and expected results files
    with open('KeyT/prompt.json', 'r') as file:
        data = json.load(file)
    expected_results = load_expected_results()

    # Get test group for tgId: 3
    test_group = next((tg for tg in data['testGroups'] if tg['tgId'] == 3), None)
    expected_group = next((tg for tg in expected_results['testGroups'] if tg['tgId'] == 3), None)
    
    if not test_group or not expected_group:
        print("Test group with tgId: 3 not found.")
        return

    # Iterate over each test case in the tgId: 3
